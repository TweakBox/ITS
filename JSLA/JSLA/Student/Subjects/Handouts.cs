...
namespace JSLA.Student.Subjects
{
    public partial class Handouts : Form
    {
        ...
        private void fetchHandouts()
        {
            ...
            {
                Usercontrols.DownloadableFileItem dfi = new Usercontrols.DownloadableFileItem(results[i, 0].ToString(), results[i, 2].ToString())
                { Title = results[i, 1].ToString(), Width = flpList.Width - 15 };
